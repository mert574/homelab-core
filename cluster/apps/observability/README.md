...
The Pulse observability stack in-cluster (namespace `observability`), synced by Argo:
...
- `dashboards-configmap.yaml` — dashboards as code, generated from the pulse-pager
  ...
  observability --from-file=<pulse-pager>/observability/grafana/dashboards/ --dry-run=client -o yaml`).
...
```
kubectl -n observability port-forward svc/pulse-grafana 3000:80
# http://localhost:3000 — admin password:
kubectl -n observability get secret pulse-grafana -o jsonpath='{.data.admin-password}' | base64 -d; echo
```
